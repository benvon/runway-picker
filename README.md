...
- `Deploy Preview`: after PR CI succeeds, a trusted `workflow_run` job downloads the immutable `dist` artifact, waits for `preview` environment approval, deploys only static Pages assets, and comments the URL. PR code never receives Cloudflare or AirportDB credentials.
- `Release Create`: after publishing a stable release for the exact successful `main` commit, directly runs the Pages and Worker production deployment jobs. Each job targets the protected `production` Environment, so production secrets are scoped to the deployment job itself.
- `Production Deployment Preflight`: manually verifies that the protected `production` Environment provides every required deployment secret and variable, reporting presence only (never values).
