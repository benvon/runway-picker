...
- `Release Create` runs after successful `CI` push checks on `main` and computes next semver from commit signals:
  ...
  - validates `RELEASE_AUTOMATION_TOKEN` with an authenticated GitHub read before any tag mutation
  - uses the highest published stable GitHub Release as its version baseline; loose/orphan tags never advance the release window
  - resolves the GitHub Release tag (including annotated tags) to its immutable commit SHA; `target_commitish` is release-creation metadata and is not used as deployment provenance
  - reconciles the intended tag, GitHub Release, and exact CI-green `main` SHA before mutation. An orphan tag at that exact SHA resumes release publication; a matching published release is idempotently complete only while it remains both the current protected `main` tip and the latest stable release. Any disagreement fails closed and requires manual investigation.
...
- `Release Create` publishes the release, then directly runs Pages and Worker production deployment jobs with the release tag and exact CI-green `main` SHA. Both jobs validate that the tag resolves to that SHA and target the protected `production` Environment.
- Release publication and production deployment remain separate concerns: a deployment failure can be retried by re-running the protected release workflow jobs while that release remains the current protected `main` tip. Once `main` advances, automatic deployment fails closed rather than redeploying an older release.
- Run `Production Deployment Preflight` from the Actions tab before enabling or changing production credentials. It uses the same direct protected Environment boundary as the deployment jobs and reports only whether each required value is present.
