...
describe('runwayParser', () => {
  it('parses runway with an explicitly magnetic heading', () => {
    expect(parseRunwayEnd('18L')).toEqual({ id: '18L', headingDegMag: 180, isClosed: false });
  });

  it('normalizes single digit runway numbers with an explicitly magnetic heading', () => {
    expect(parseRunwayEnd('9')).toEqual({ id: '09', headingDegMag: 90, isClosed: false });
  });

  it('deduplicates a magnetic runway input list', () => {
